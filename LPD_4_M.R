...
Join_LPD4 <- rename(Join_LPD1, c("meanlogFC" = "log2FoldChange_LPD7",
...
Join_LPD4_melt <- melt(Join_LPD4, id.vars = c("Gene_Symbol"))

Join_LPD4_melt_abs <- Join_LPD4_melt %>%
  mutate(abs_Log2FC = abs(value))


Join_LPD4_melt_abs_meangamma <- Join_LPD4_melt_abs %>%
...
Ordered_Gene_LPD4 <- Join_LPD4_melt_abs_meangamma %>%
  arrange(Gene_Symbol)

NA_Gene_LPD4 <- na.omit(Ordered_Gene_LPD4)

GeneVector <- as.character(unique(NA_Gene_LPD4$Gene_Symbol))
...
Sig_genes_LPD4 <- Meth_expression_LPD4 %>%
  dplyr::filter(Gene_Symbol %in% c("CTD-2231E14.2", "MIR4444-1","PSMB9",
                                   "TPM4", "TTK"))
