...
Join_LPD6 <- rename(Join_LPD1, c("log2FoldChange" = "log2FoldChange_LPD7",
...
Join_LPD6_melt <- melt(Join_LPD6, id.vars = c("Gene"))

Join_LPD6_melt_abs <- Join_LPD6_melt %>%
  mutate(abs_Log2FC = abs(value))

Join_LP6_melt_abs_meangamma <- Join_LPD6_melt_abs %>%
...
Ordered_Gene_LPD6 <- Join_LP6_melt_abs_meangamma %>%
  arrange(Gene)

NA_Gene_LPD6 <- na.omit(Ordered_Gene_LPD6)

GeneVector <- as.character(unique(NA_Gene_LPD6$Gene))
...
Sig.Genes <- NA_Gene_LPD6 %>%
  dplyr::filter(Gene %in% c("C11orf54",
                            "CCDC28A",
                            "COX20P1",
                            "DRICH1",
                            "CHURC1-FNTB",
...
Sig_genes_LPD6 <- Gene_expression_LPD6 %>%
  dplyr::filter(Gene %in% c("C11orf54",
                            "CCDC28A",
                            "COX20P1",
                            "DRICH1",
