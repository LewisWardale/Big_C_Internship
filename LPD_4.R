...
Join_LPD4 <- rename(Join_LPD1, c("log2FoldChange" = "log2FoldChange_LPD7",
...
Join_LPD4_melt <- melt(Join_LPD4, id.vars = c("Gene"))

Join_LPD4_melt_abs <- Join_LPD4_melt %>%
  mutate(abs_Log2FC = abs(value))

Join_LP4_melt_abs_meangamma <- Join_LPD4_melt_abs %>%
  mutate(meangamma = case_when(
    variable == "log2FoldChange_LPD7" ~ 0.09874510,
    variable == "log2FoldChange_LPD6" ~ 0.05574839,
    variable == "log2FoldChange_LPD5" ~ 0.09885841,
    variable == "log2FoldChange_LPD4" ~ 0.54801097,
    variable == "log2FoldChange_LPD3" ~ 0.09401900,
    variable == "log2FoldChange_LPD2" ~ 0.06594061,
    variable == "log2FoldChange_LPD1" ~ 0.03867753,
  ) )


Ordered_Gene_LPD4 <- Join_LP4_melt_abs_meangamma %>%
  arrange(Gene)

NA_Gene_LPD4 <- na.omit(Ordered_Gene_LPD4)

GeneVector <- as.character(unique(NA_Gene_LPD4$Gene))

for (Gene1 in GeneVector) {
  
  #Filters for each Gene
  Single_gene <- NA_Gene_LPD4 %>%
...
Sig.Genes <- NA_Gene_LPD4 %>%
  dplyr::filter(Gene %in% c("CUL7",
                            "HNRNPA3P7",
                            "LINC02023",
                            "MAP6",
                            "LINC02706",
...
write.csv(Sig.Genes, "Sig_genes_LPD_4.csv")
...
for (Gene1 in GeneVector2) {
  ...
  pdf(paste0("LPD_4/Scatterplot_LPD4_",Gene1,".pdf"))
  print(superplot)
  dev.off()
}  

Sig.Genes2 <- Sig.Genes %>%
  dplyr::filter(Gene == "RNU6-157P")
...
Sig_genes_LPD4 <- Gene_expression_LPD4 %>%
  dplyr::filter(Gene %in% c("CUL7",
                            "HNRNPA3P7",
                            "LINC02023",
                            "MAP6",
...
Sig_diff_exp_LPD4 <- Sig_genes_LPD4 %>%
  dplyr::filter(abs(log2FoldChange) >= 1) %>%
  dplyr::filter(padj <= 0.05) %>%
  dplyr::filter(!str_detect(Gene, "^ENSG")) %>%
  select(Gene,log2FoldChange,padj)

write.csv(Sig_diff_exp_LPD4,"Sig_diff_genes_LPD_4.csv")  
