...
Gamma_Gene_count_LPD4 <- read.csv("/Users/lewiswardale/Desktop/Gene_count/Gene_count/correlationResult_LPD_4.csv") %>%
  mutate(Gene = gsub("(.*)\\..*", "\\1", Gene)) 

GeneSymbol = bitr(Gamma_Gene_count_LPD4$Gene, fromType = "ENSEMBL", toType = "SYMBOL", OrgDb = "org.Hs.eg.db") %>%
  left_join(Gamma_Gene_count_LPD4, by = c("ENSEMBL"  = "Gene"))
...
Gamma_samples <- dplyr::select(Gamma_samples, LPD_4, Sample)
# Joining -----------------------------------------------------------------
...
superplot <- ggplot(Joined, aes(x = LPD_4, y = Gene_Count)) +
  geom_point(shape = 1) + 
