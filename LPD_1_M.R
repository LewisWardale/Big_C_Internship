...
length(unique(Meth_expression_LPD1$Gene_Symbol)) == nrow(Meth_expression_LPD1)
...
g <- Meth_expression_LPD1 %>%
  filter(Gene_Symbol %in% unique(.[["Gene_Symbol"]][duplicated(.[["Gene_Symbol"]])]))
...
Join_LPD1 <- rename(Join_LPD1, c("meanlogFC" = "log2FoldChange_LPD7",
...
Join_LPD1_melt <- melt(Join_LPD1, id.vars = c("Gene_Symbol"))
...
GeneVector <- as.character(unique(NA_Gene_LPD1$Gene_Symbol))
...
Sig.Genes <- NA_Gene_LPD1 %>%
  dplyr::filter(Gene_Symbol %in% c("Y_RNA", "TIPIN", "RP5-881L22.5",
                            "PPP2R2D","KLHL3", "HNF4A", "ECSIT",
                            "COMMD9", "CHM", "BMP15", "BAD", "ACSBG1"))
...
Sig_genes_LPD1 <- Meth_expression_LPD1 %>%
  dplyr::filter(Gene_Symbol %in% c("Y_RNA", "TIPIN", "RP5-881L22.5",
                            "PPP2R2D","KLHL3", "HNF4A", "ECSIT",
                            "COMMD9", "CHM", "BMP15", "BAD", "ACSBG1"))
