...
Join_LPD3 <- rename(Join_LPD1, c("meanlogFC" = "log2FoldChange_LPD7",
...
Join_LPD3_melt <- melt(Join_LPD3, id.vars = c("Gene_Symbol"))

Join_LPD3_melt_abs <- Join_LPD3_melt %>%
  mutate(abs_Log2FC = abs(value))


Join_LPD3_melt_abs_meangamma <- Join_LPD3_melt_abs %>%
...
Ordered_Gene_LPD3 <- Join_LPD3_melt_abs_meangamma %>%
  arrange(Gene_Symbol)

NA_Gene_LPD3 <- na.omit(Ordered_Gene_LPD3)

GeneVector <- as.character(unique(NA_Gene_LPD3$Gene_Symbol))
...
Sig.Genes <- NA_Gene_LPD3 %>%
  dplyr::filter(Gene_Symbol %in% c("AC026904.1", "CALML3", "CALML3-AS1",
                                   "RP11-770E5.1"))
...
ggplot(Sig.Genes, aes(x = meangamma , y = abs_Log2FC, color = Gene_Symbol)) +
  ...
  geom_text(label = summary(m)$r.squared, x = 0.3, y = 0.5, show.legend = FALSE)

# Sig diff genes 2 ------------------------------------------------------------------

Sig_genes_LPD3 <- Meth_expression_LPD3 %>%
  dplyr::filter(Gene_Symbol %in% c("AC026904.1", "CALML3", "CALML3-AS1",
                                   "RP11-770E5.1"))
